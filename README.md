...
This repository contains a clean, reproducible research-code implementation for studying feature-level explanations in heavy rainfall event prediction. The project trains a Random Forest Classifier on a **synthetic meteorological dataset** and uses SHAP values to interpret which factors contribute most strongly to predicted heavy rainfall events.
...
Heavy rainfall events can create severe social, environmental, and infrastructure risks. Machine learning models can support early warning research, but this repository is **research code, not operational weather forecasting**. It is intended for reproducible experimentation, explainability analysis, and project presentation.
...
The target label is created from a transparent synthetic risk score. This keeps the project reproducible while avoiding false claims about access to real restricted meteorological data.
...
These values are produced on the synthetic dataset and should not be interpreted as real-world forecasting performance.
...
- Modular scripts for dataset generation, training, evaluation, and explainability
- Outputs saved as reusable visual artifacts
- Designed for reproducible research presentation
